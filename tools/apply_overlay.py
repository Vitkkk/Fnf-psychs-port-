...
All edits are anchor checked. If the pinned upstream source differs, CI fails instead
of silently producing a partially patched APK.
"""
...
def insert_once(path: Path, anchor: str, insertion: str, before: bool = True) -> None:
    text = path.read_text(encoding="utf-8")
...
def main() -> None:
    ...
    # Android defaults to the new Week Editor. Classic remains available inside it.
    ...
    # Snapshot original grid edits so undo/redo includes touch-created notes and deletes.
    insert_once(
        chart,
	...
	public function mobileChangeSection(delta:Int):Void {
		...
		if (target >= _song.notes.length) {
			addSection();
			target = _song.notes.length - 1;
		}
	...
	public function mobileCopySelected():Void {
		if (curSelectedNote != null) mobileCopiedNote = curSelectedNote.copy();
	}

	public function mobilePasteSelected():Void {
		if (mobileCopiedNote == null) return;
		var copy = mobileCopiedNote.copy();
		copy[0] = Conductor.songPosition;
		_song.notes[curSection].sectionNotes.push(copy);
		curSelectedNote = copy;
		updateGrid();
		updateNoteUI();
	}
	...
	public function mobileSetSongFields(name:String, ?bpm:Null<Float>, ?speed:Null<Float>):Void {
		if (name != null && name.length > 0) {
			_song.song = name;
			if (UI_songTitle != null) UI_songTitle.text = name;
			currentSongName = Paths.formatToSongPath(name);
		}
		if (bpm != null) {
			_song.bpm = bpm;
			tempBpm = bpm;
	...
	public function mobileSetSongCharacter(slot:String, id:String):Void {
		switch(slot) {
			case 'bf': _song.player1 = id;
			case 'gf': _song.player3 = id;
			default: _song.player2 = id;
		}
		updateHeads();
	}
	...
	public function mobileToggleSectionFlag(field:String):Void {
		...
		switch(field) {
			case 'mustHitSection': sec.mustHitSection = !sec.mustHitSection;
			case 'gfSection': sec.gfSection = !sec.gfSection;
			case 'altAnim': sec.altAnim = !sec.altAnim;
			case 'changeBPM': sec.changeBPM = !sec.changeBPM;
		}
		updateGrid();
		updateSectionUI();
	}

	public function mobileCopySection():Void {
		mobileCopiedSection = [];
		for (note in _song.notes[curSection].sectionNotes) mobileCopiedSection.push(note.copy());
	}

	public function mobilePasteSection():Void {
		if (mobileCopiedSection == null) return;
		_song.notes[curSection].sectionNotes = [];
		for (note in mobileCopiedSection) _song.notes[curSection].sectionNotes.push(note.copy());
		updateGrid();
	}
	...
	public function mobileEventsForCurrentSection():Array<Array<Dynamic>> {
		...
		for (note in _song.notes[curSection].sectionNotes)
			if (note != null && note.length >= 5 && note[1] < 0) result.push(cast note);
		return result;
	}
	...
	public function mobileAddEvent(name:String, value1:String, value2:String):Void {
		...
		_song.notes[curSection].sectionNotes.push(event);
		curSelectedNote = event;
		updateGrid();
		updateNoteUI();
	}
	...
	public function mobileDeleteEvent(event:Array<Dynamic>):Void {
		_song.notes[curSection].sectionNotes.remove(event);
		if (curSelectedNote == event) curSelectedNote = null;
		updateGrid();
		updateNoteUI();
	}
	...
	public function mobileReplaceSong(song:SwagSong):Void {
		_song = song;
		PlayState.SONG = song;
		if (UI_songTitle != null) UI_songTitle.text = song.song;
...
'''
    ...
    # Existing Save button must also respect Android direct-to-mod behavior.
    replace_once(
        chart,
    ...
    # On Android a mod JSON is a filesystem path, not an OpenFL asset id.
